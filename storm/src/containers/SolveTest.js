    ...
    render(){
        ...
        return(
            <div>
                ...
                {tab.map(()=>{
                    if(numberOfQuestion != this.state.iterator){return(
                    <div>
                    ...
                    {this.state.dane.questions[this.state.iterator].answer.map((answ,i)=>{
                        if(i !=0){this.state.flaga=false
                            return( <form onClick={this.handleSubmit}><input type="radio" name="radio" value={answ}/>{answ}</form>)}
                    })}
                    {tab.map(()=>{if(this.state.flaga){return(<div>
                    <form>
                ...
                else{return(<button onClick={this.routeChange}>Zakoncz test</button>)}}
                )}
